...
#if defined(__X86_64__) || defined(__x86_64__) || defined(__i386__) || defined(__aarch64__) || defined(__arm64__) || (defined(__riscv) && defined(__riscv_zicclsm))
...
#if defined(__e2k__)
static inline uint64_t sip_load64_le_aligned(const uint8_t *p)
{
    uint64_t v;
    const uint8_t *pa = (const uint8_t *)__builtin_assume_aligned(p, 8);
    __builtin_memcpy(&v, pa, sizeof(v));
    return v;
}
#endif

static inline uint64_t sip_load64_le_portable(const uint8_t *p)
{
    return U8TO64_LE(p);
}
...
uint64_t siphash(const uint8_t *in, const size_t inlen, const uint8_t *k)
{
    ...
    if ((((uintptr_t)k) & 7) == 0)
    {
        k0 = sip_load64_le_aligned(k);
        k1 = sip_load64_le_aligned(k + 8);
    }
    else
#endif
    {
        k0 = sip_load64_le_portable(k);
        k1 = sip_load64_le_portable(k + 8);
    }
